...
The server entry connects the generated route importer, route runtime, and
Octane document service. `target: "node"` selects the built-in srvx adapter for
this example, and the entry must default-export the composed result:
...
- `dist/server/server.js` default-exports the selected Web Fetch or adapter
  server entry.
...
load the default export from `dist/server/server.js` and expose its `fetch`
function directly; an srvx-targeted entry also has srvx middleware and options.
...
| The server entry validation fails                          | Default-export the value returned by `createServerEntry`; named exports are not consumed by `ff`.                         |
...
`ff build` emits client assets and `dist/server/server.js`, then pre-renders
every static route. The generated server entry always exposes the Web Fetch
contract plus the mode-aware document and route-data operations used by the
lifecycle. A runtime target can add srvx's server options and static asset
middleware around that contract.

`ff dev`, `ff build`, and `ff preview` consume the default export directly;
named server exports are not part of the contract.

## Server output targets

The output choice belongs in the `flamefront` Vite plugin options. `srvx` is an
...
export default createServerEntry({
  ...
  documents,
  assets: {
    loadTemplate: () => templateFromTheHost(),
    loadStaticFragment: ({ route }) => loadFragmentFromTheHost(route.path),
  },
})
```

For Node, Deno, or Bun apps that want Flamefront to serve the built client
directory, set `target` and use `assets.clientDirectory` as shown below. The
public `createServerEntry` name stays the same across output choices.
...
document service, and selected server adapter:
...
- `createFetchServerEntry({ app, documents, assets, middleware?, headers? })`
  owns the Web `fetch` handler, template lookup, route-data dispatch,
  render-mode dispatch, fragment loading, and default response headers. Its
  `assets.loadTemplate` callback keeps the handler independent of a filesystem.
- `createSrvxServerEntry({ app, documents, assets, middleware?, headers? })`
  adds srvx's static asset middleware and filesystem-backed template and
  fragment lookup. The required `assets.clientDirectory` locates client files.
  The generated `createServerEntry` alias selects one of these implementations
  from the Vite output options.
...
semantics, the Web transport, and the adapter-specific asset behavior.

Configure shared paths on the app definition so matching, generated browser
routes, the server router, the data endpoint, and every server adapter use the
same values:
...
- raw TypeScript exports for the route manifest, Vite integration, Web Fetch and
  srvx server entries, output configuration, Octane browser entry, and Remix
  Router adapter;
