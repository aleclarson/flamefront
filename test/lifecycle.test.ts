...
test("prerenders every supplied static route", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      JSON.parse(
        await readFile(staticRouteDataFile(clientDirectory, routes[0]), "utf8"),
      ),
      { pathname: "/docs" },
    )
    assert.deepEqual(
      JSON.parse(
        await readFile(
          resolve(clientDirectory, "about/team/index.data.json"),
          "utf8",
        ),
      ),
      { pathname: "/about/team" },
...
test("rejects parameterized static routes without concrete paths", async () => {
  const root = await mkdtemp(resolve(tmpdir(), "flamefront-build-"))
  const parameterizedRoute = route("/articles/:slug", "/src/Article.tsrx", {
    render: "static",
  })
...
test("writes rendered static route data without running a fallback loader", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      JSON.parse(
        await readFile(
          staticRouteDataFile(clientDirectory, staticRoute),
          "utf8",
        ),
      ),
      { source: "render" },
