...
test("normalizes shared basename and route-data paths", async () => {
  ...
  try {
    assert.deepEqual(await app.load("https://example.test/docs/guide"), {
      loaded: true,
    })
...
test("selects matches by render mode", () => {
  ...
  assert.equal(
    app.match("/articles/new", { render: "server" })?.data.entry,
    "/src/Article.tsrx",
  )
...
test("deduplicates load and prefetch requests", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      await app.load("https://example.test/data", { reload: true }),
      { value: 2 },
    )
