...
`,
  ...
  const clientContext = {
    ...
    resolve: async (id: string) => ({
      id: path.join(root, "src", id.replace("./", "")),
    }),
  }
...
export default function Route() {}`
  ...
  try {
    const transformed = await routePlugin.transform.call(
      clientContext,
      source,
      routeId,
    )
...
test("rejects server modules that remain in the client graph", async () => {
  ...
  try {
    await assert.rejects(
      frameworkPlugin.resolveId.call(
        clientContext,
        "./data.server.ts",
        routeId,
      ),
...
test("omits mixed route sources from emitted client source maps", () => {
  ...
  assert.deepEqual(transformed.sourcesContent, [
    null,
    `export const shared = 'client';`,
  ])
})
...
test("generates an eager shell root with lazy layouts and route metadata", () => {
  ...
  assert.equal(source.match(/import\("\/src\/Shell\.tsrx"\)/g)?.length, 2)
  ...
  assert.match(
    source,
    /"render":"server","navigation":"fragment","hydration":"deferred"/,
  )
  assert.match(
    source,
    /"render":"server","navigation":"fragment","hydration":\{"when":"visible","rootMargin":"200px"\}/,
  )
  assert.match(
    source,
    /loadRouteFragment\(args, \{"basename":"\/","dataPath":"\/__flamefront\/data"\}, "server"\)/,
  )
  ...
  assert.match(
    source,
    /const componentModule = await import\("\/@flamefront\/hydration-route/,
  )
  assert.match(
    source,
    /if \(import\.meta\.env\.SSR\) return \{\}; const routeModule = await import\("\/src\/Client\.tsrx"\)/,
  )
  ...
  assert.doesNotMatch(source, /"\/src\/Server\.tsrx": \(\) =>/)
  assert.doesNotMatch(source, /"\/src\/Visible\.tsrx": \(\) =>/)
...
test("marks static routes for fragment navigation and excludes them from module preloaders", () => {
  ...
  assert.match(
    source,
    /loader: \(args\) => loadRouteFragment\(args, \{"basename":"\/","dataPath":"\/__flamefront\/data"\}, "static"\)/,
  )
  assert.match(source, /hydration-route\.tsrx\?entry=%2Fsrc%2FAbout\.tsrx/)
  assert.doesNotMatch(
    source,
    /hydration-route\.tsrx\?entry=%2Fsrc%2FBuiltFull\.tsrx/,
  )
  assert.doesNotMatch(
    source,
    /hydration-route\.tsrx\?entry=%2Fsrc%2FBuiltDeferred\.tsrx/,
  )
  assert.match(
    source,
...
test("resolves the public generated routes module and hydration TSRX modules", async () => {
  ...
  const serverGeneratedId = await plugin.resolveId.call(
    pluginContext,
    serverRoutesId,
  )
