...
test("shares a live data prefetch with generated client route loaders", async () => {
  ...
  globalThis.fetch = async (input) => {
    ...
    assert.equal(
      ...
      "https://example.test/server-prefetch",
    )
  ...
  const app = defineApp({
    ...
    routes: [
      route("/server-prefetch", "/src/Server.tsrx", { render: "server" }),
    ],
  })
...
test("prefetches static route data from the build artifact", async () => {
  ...
  globalThis.fetch = async (input) => {
    ...
    assert.equal(
      String(input),
      "https://example.test/static-prefetch/index.data.json",
    )
  ...
  const app = defineApp({
    shell,
    routes: [
      route("/static-prefetch", "/src/Static.tsrx", { render: "static" }),
    ],
  })
