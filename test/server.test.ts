...
test("loads the matched route module with request parameters", async () => {
  ...
  const request = new Request(
    "https://example.test/articles/hello%20world?preview=true",
  )
...
test("creates a srvx handler for every render mode", async () => {
  ...
  try {
    const rootResponse = await server.fetch(
      new Request("http://flamefront.test/"),
    )

    assert.equal(rootResponse.status, 302)
    assert.equal(rootResponse.headers.get("location"), "/client")

    const clientResponse = await server.fetch(
      new Request("http://flamefront.test/client"),
    )

    assert.equal(
      await clientResponse.text(),
      "<main>Client shell rendered</main>",
    )

    const serverResponse = await server.fetch(
      new Request("http://flamefront.test/server"),
    )

    assert.equal(await serverResponse.text(), "<main>SSR page</main>")

    const staticResponse = await server.fetch(
      new Request("http://flamefront.test/static"),
    )
